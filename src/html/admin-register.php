...
if (isset($_POST['registerBtn'])) {
    ...
    $checkQuery = "SELECT * FROM admin WHERE adminID = '$adminID'";
    $checkResult = mysqli_query($con, $checkQuery);

    if (mysqli_num_rows($checkResult) > 0) {
        echo '<p class="text-center text-danger">Admin ID already exists. Please choose a different one.</p>';
    } else {
        // Insert new admin
        $insertQuery = "INSERT INTO admin (adminID, password, fullname) VALUES ('$adminID', '$password', '$fullname')";
...
?>
                ...
                <form method="post">
                  ...
                  <button type="submit" name="registerBtn" class="btn btn-primary w-100 py-8 fs-4 mb-3 rounded-2" style="background-color: #F78F1E; color: #fff;">Register</button>
                  <div class="d-flex align-items-center justify-content-center">
                    <p class="fs-4 mb-0 fw-bold">Already Has An Account?</p>
                    <a class="text-primary fw-bold ms-2" href="./admin-login.php">Log in</a>
                  </div>
                </form>
              </div>
            </div>
